...
use joiner::{ContactMeaning, compose, measure_contact_geometry};

use super::{WIDTH, purlin_key, study};
...
impl Fixture {
    fn new(depth_mm: u32) -> Result<Self> {
        ...
        for (key, family) in &frame.instances {
            if assembly.part_by_key(family).is_none() {
                let element = frame
                    .construction
                    .element(key)
                    .ok_or("missing study element")?;
                assembly.add_recipe_part(family, compose(&frame.construction, element)?)?;
            }
        }
    ...
    fn part(&self, key: &str) -> &CompiledPart {
        let (_, family) = self
            ...
            .unwrap();
        self.compiled
            .part(self.assembly.part_by_key(family).unwrap())
            .unwrap()
    }
...
fn every_rafter_has_two_full_bearings_and_each_lap_has_real_side_contact() -> Result<()> {
    for depth in [2_400, 3_601, 5_400] {
        ...
        assert!(fixture.frame.verify(&fixture.assembly, &fixture.compiled)? > 0.0);
        let mut bearings = 0;
        let mut laps = 0;
