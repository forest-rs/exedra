...
def main():
    ...
    roof_top = max((obj.matrix_world @ Vector(corner)).z
    ...
    views = [
        ...
        ("ridge-uphill", (0.25, -depth / 2 - 1.9, roof_top - 0.97),
         (0.25, 0.0, roof_top - 0.25), 62),
        ("ridge-end", (width / 2 + 1.2, -0.7, roof_top - 0.23),
         (width / 2 - 0.8, 0.0, roof_top - 0.23), 48),
    ]
    if case == "exploded":
    ...
    if len(args) > 2:
        views = [view for view in views if view[0] in args[2:]]
        if not views:
