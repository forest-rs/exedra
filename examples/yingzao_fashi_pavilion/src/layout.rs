...
pub(crate) const CROSS_BEAM_WIDTH: f64 = 0.24;
...
impl Layout {
    pub(crate) fn column_top(&self) -> f64 {
        0.54 + self.column_height
    }

    /// Upper arm's base measured from the column top, after the crossed lap.
    pub(crate) fn upper_arm_offset(&self) -> f64 {
        0.15 + self.arm_depth - 0.045
    }

    pub(crate) fn upper_arm_top(&self) -> f64 {
        self.column_top() + self.upper_arm_offset() + self.arm_depth
    }
