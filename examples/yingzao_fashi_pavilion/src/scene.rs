...
use crate::{
    Result, brackets, geometry,
    joinery::FittedConstruction,
    layout::{CROSS_BEAM_WIDTH, Layout},
    rafters,
...
fn frame(scene: &mut Scene, l: &Layout) -> Result<()> {
    ...
    let plate = scene.part(
        "longitudinal-beam",
        geometry::block([l.span - CROSS_BEAM_WIDTH, 0.22, 0.24])?,
        "timber",
    )?;
    let top = l.column_top();
    ...
    for (i, pair) in l.frames.windows(2).enumerate() {
        for (side, y) in [("front", -l.depth * 0.5), ("back", l.depth * 0.5)] {
            scene.place(
                ...
                plate,
                // Butt against the transverse beam; the tip blocks below
                // provide bearing without intersecting the crossed arms.
                [pair[0] + CROSS_BEAM_WIDTH * 0.5, y - 0.11, top + 0.495],
            )?;
        }
    }
