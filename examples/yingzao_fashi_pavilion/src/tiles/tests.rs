...
use super::ridge::{LIFT as RIDGE_LIFT, RADIUS as RIDGE_RADIUS};
use super::*;
...
fn tile_laps_clear_the_clay_and_decking_across_pitch_changes() -> Result<()> {
    for depth_mm in [2_400, 2_700, 3_601, 4_500, 5_400] {
        ...
        let final_cap = assembly.add_recipe_part("final-cap", ridge::terminal_shell(true)?)?;
        let final_pan = assembly.add_recipe_part("final-pan", ridge::terminal_shell(false)?)?;
        ...
        for (name, part, terminal, spacing, lift, x, projection) in [
            (
                "cap",
                cap,
                final_cap,
...
fn ridge_shells_nest_and_clear_the_last_roof_courses() -> Result<()> {
    ...
    let cap = assembly.add_recipe_part("cap", ridge::terminal_shell(true)?)?;
    let pan = assembly.add_recipe_part("pan", ridge::terminal_shell(false)?)?;
    let compiled = PartCompiler::new().compile_parts(&assembly, &compile_policy())?;
    check_reports(&assembly, &compiled)?;
...
#[test]
fn ridge_closes_the_valley_below_its_crown() -> Result<()> {
    let section = RoofSection::new(&Layout::resolve(Parameters::default())?);
    let mut assembly = Assembly::new();
    let foot = assembly.add_recipe_part("foot", ridge::foot(&section, 0.225, false)?)?;
    let compiled = PartCompiler::new().compile_parts(&assembly, &compile_policy())?;
    check_reports(&assembly, &compiled)?;
    let foot = Solid::new(compiled.part(foot).unwrap(), Placement3::IDENTITY);
    assert!(
        foot.contains([0.1125, -0.12, 0.08]),
        "daylight between the roof valleys and ridge crown"
    );
    Ok(())
}
