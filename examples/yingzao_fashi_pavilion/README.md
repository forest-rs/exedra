...
blender --background --python examples/yingzao_fashi_pavilion/tools/render.py -- target/yingzao-fashi-pavilion pavilion ridge-uphill ridge-end
...
`brackets.png`, `roof.png`, `eaves.png`, `ridge-uphill.png`, `ridge-end.png`, and `pavilion.blend`; other cases prefix their image
...
tile layer is a child of the roof. The default has 53 parts, 1,568 geometry
instances, 17 spatial frames and 2,052 placed bodies. Moving the roof carries
...
The first transverse beams bear on the upper bracket arms; their depths follow
that bearing plane while their tops retain the roof datums. Longitudinal beams
bear on the bracket tip blocks and butt against the transverse beam faces.
Tests check positive surface coverage and non-interference at those junctions
through the exported instance placements, including opposing dimension limits.
These are explicit bearing and butt interfaces, without beam-end tenons.
...
aprons finish the eaves. The final cover and pan courses use constant sections
where fitted ridge-foot pieces meet them.

`tiles/ridge.rs` builds an authored main ridge (*dàjǐ*): repeated ridge-foot
closures, a low fillet course, curved crown tiles and closed end plates. The
closure's shaped facing and solid backing are represented together. Its lower
contour shares polygon vertices with the terminal roof tiles; those facets have
at most 0.09 mm chord error. Retained extrusion-to-plane and capped cuts join the
two roof halves and establish a horizontal bed under the fillet. The end fillet
extends under the end plate too. These pieces close the formerly open valleys
beneath the crown without intersecting the terminal clay. Ordinary roof-course
bedding remains unmodeled.

Roofing tests check laps, decking clearance and outward winding. Contact tests
use the same flattened placements as GLB export: terminal clay to ridge foot,
ridge foot to fillet, and both feet of every crown tile and both end plates.
They include near-corner samples, cut-bed area/extents, mixed dimension extremes,
and a deliberately lifted closure that must fail its contact witnesses. These
are explicit interface checks and finite collision samples, not a general
collision or weatherproofing certificate.
...
The ridge-foot closures are informed by the Palace Museum's description of
[*dangou* tiles at the ridge/roof junction](https://www.dpm.org.cn/Uploads/file/2025/02/13/1739432353eIvyvIZcw260.pdf),
which also discusses the grooved terminal covers described in *Yingzao Fashi*.
The example authors its own fitted contours and dimensions; it does not reproduce
that historical groove or a particular ridge's full sequence of components.
