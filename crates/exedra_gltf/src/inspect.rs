...
impl GlbDocument {
    ...
    /// named `name`, in level order. Returns `None` when no node has that
    /// name, the node has no `MSFT_lod` extension, or any listed id is out of
    /// range or names an unnamed node.
    #[must_use]
