...
5. **Chains are opt-in through `GltfLods::MsftLod`.** The default,
   ...
   swaps its meshes. A node-replacing viewer switches levels. A
   mesh-swapping viewer finds no mesh on the extension node of a multi-body
   or instanced chain (its children carry the meshes) and keeps drawing
   level 0: a graceful degradation, not a level switch.
...
7. **Instanced chains share transforms.** A batch whose part has a chain
   becomes a group node carrying the batch identity and `MSFT_lod`, keeping
   the batch name (so `lod_node_names` takes `… [2 instances]` here, and
   `… [lod 0]` in the node layout). Its children are the per-body instanced
   nodes of level 0, `… [lod 0] [body i]`. Each lower level is a group
   `… [lod k]` of per-body instanced nodes `… [lod k] [body i]` that
   reference the same transform accessors; bodies are always suffixed, so a
   level group never shares a name with its only body. Batching then groups
   instances by the material resolution of every written level.
...
9. **Sheared placements keep their authored matrix.** glTF requires node
   matrices to decompose into TRS, and the Khronos validator reports sheared
   ones (`NODE_MATRIX_NON_TRS`). Exedra writes the placement as authored
   rather than approximating it, in both instancing modes, and counts every
   such node in `GltfStats::non_trs_matrices`. Refusing or approximating
   would silently change geometry.
10. **Extension listing.** `MSFT_lod` is listed as used, never as required:
