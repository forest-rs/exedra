...
impl PatchLoop {
    ...
    #[must_use]
    pub fn sources(&self) -> &[BoundarySource] {
        &self.sources
    }
}
...
/// that snapshot only, not persistent selections across edits.
#[derive(Clone, Debug, PartialEq)]
...
impl PlanarPatch {
    ...
    /// keep this association automatically.
    ///
    /// # Errors
    /// Rejects stale source data, invalid boundaries, numeric limits and budgets.
    pub fn from_workplane(
        ...
        for boundary in boundaries {
            ...
            for edge in boundary {
                ...
                let face = body.mesh.face(edge).ok_or(Error::InvalidBoundary)?;
                let feature = body
                    .source_map
                    .face_feature(face)
                    .ok_or(Error::InvalidBoundary)?;
                let adjacent_feature = body
                    .mesh
                    .twin(edge)
                    .and_then(|e| body.mesh.face(e))
                    .and_then(|f| body.source_map.face_feature(f));
                points.push(xy);
    ...
    pub fn circle_clearance(
        ...
        let mut best: Option<(f64, BoundaryWitness)> = None;
            ...
            for (segment_index, &a) in ring.points.iter().enumerate() {
                ...
                if best.is_none_or(|(previous, _)| distance < previous) {
                    best = Some((
                        distance,
                        BoundaryWitness {
                            loop_index: crate::len_u32(loop_index),
                            segment_index: crate::len_u32(segment_index),
                            point,
