...
//!
//! Use [`exedra_mesh_ops`] for direct mesh modeling and geometric queries.
//! This crate adds reusable command plans, stale-plan checks, clone-based
//! previews, diagnostics, timings and change reports around those operations.
//! Geometry algorithms stay in mesh ops; topology and primitive edits stay in
//! [`exedra_mesh`]. Constructive and analytic callers use their native APIs.
//!
//! [`EditOperator`] and [`OperatorRunner`] are mesh-specific. This is not a
//! heterogeneous graph or scheduler. The default feature is `std`; `libm`
//! supports `no_std` applications. The facade exposes this crate only through
//! its opt-in `edit` feature.
//!
//! Tagging note:
...
//!
//! The intended public surface is this crate root (`exedra_edit::...`) through
//! re-exported operator/runtime types and functions.
//!
//! For manual docs, see:
//! - [`manual::reference`] for the operator reference
...
//! - Operator reference + authoring map: [`manual::reference`], [`manual::operators`]
//!
...
compile_error!("exedra_edit requires either the `std` or `libm` feature");
...
pub use policy::{LimitsPolicy, PolicySet, PropagatePolicy, ValidatePolicy};
pub use selection::{
