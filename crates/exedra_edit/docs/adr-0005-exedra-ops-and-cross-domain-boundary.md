...
Name the implemented SDK `exedra_edit` and keep its present capability honest.
...
- typed workflow adapters, including the domain-neutral pattern planning and
  expansion in `exedra_edit::assembly`.
...
The default `exedra_edit` build is the mesh-operation surface with `std`. Native
...
Both source kinds may coexist in one assembly, and `exedra_edit::assembly`
...
a `Mesh`, apply one `exedra_edit` edit, branch one upstream result to two
