# ADR-0001: Exedra Edit SDK Surface
...
mesh-operations crate (`exedra_edit`). The kernel owns polygon state and its
...
Sibling domain crates own their native values and lifecycles. `exedra_edit` has
...
- Exedra Edit is the workflow-facing mesh-operations surface.
- Exedra Edit may expose a typed adapter when a caller explicitly crosses into
...
Exedra Edit crate docs and manual are the primary entry point for mesh
workflows. Exedra crate docs identify Exedra as engine tier and point workflow
users to Exedra Edit. Cross-domain limits and the future typed-graph seam are
documented in ADR-0005.

Curated Exedra Edit re-exports include common workflow-facing engine types
(`Mesh`, ID types, `BuildParams`, `ExtractParams`, `DeletePolicy`) so typical
workflow code can remain in `exedra_edit::...` imports.
...
- Some mesh-workflow conveniences that are easy to add in Exedra are
  intentionally routed through Exedra Edit, adding minor coordination overhead.
