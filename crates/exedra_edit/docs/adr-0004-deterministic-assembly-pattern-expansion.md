...
Add `exedra_edit::assembly`, backed by the existing workspace
`exedra_assembly` crate.

Fence: `exedra_edit::assembly` owns deterministic repeat/distribute planning and
...
in `NodeKind::Mirror`. Exedra Edit can then register or reuse a distinct named
recipe-backed part and place it using only proper-rigid transforms.

## Consequences

- `exedra_edit` gains one existing-workspace production dependency on
