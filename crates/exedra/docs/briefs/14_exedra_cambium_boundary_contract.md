# Brief: Exedra/Cambium boundary contract (kernel promises and operator responsibilities)

## Decision
Exedra and Cambium have an explicit boundary contract:

- **Exedra** is the calm kernel: topology + attributes + deterministic extraction + validation + (later) booleans.
- **Cambium** is the operator layer: workflows, tool semantics, procedural generation, preview/commit orchestration.

Cambium never depends on Exedra internals; Exedra never depends on Cambium concepts.

## Why
Without a boundary contract:
- Cambium reaches into kernel data structures
...
- Cambium moves faster by adding operators without changing Exedra.
- Debuggability is shared: Exedra provides core reports/artifacts; Cambium provides operator reports/artifacts.
