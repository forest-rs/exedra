...
///
/// Input to direct modeling operations and region queries.
...
/// Canonical undirected edges resolved against one mesh.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ResolvedEdgeSet {
    /// Sorted, deduplicated canonical representatives of live twin pairs.
    pub edges: EdgeSet,
    /// Number of sort/dedup passes that changed IDs: input and topology-mapped.
    /// Ranges from zero to two; mapping a single twin alone does not count.
    pub canonicalization_steps: u8,
}

/// A selected half-edge has no live canonical twin pair.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct EdgeSelectionError {
    /// Rejected half-edge.
    pub edge: HalfEdgeId,
}
impl fmt::Display for EdgeSelectionError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "selected half-edge {:?} has no live twin pair",
            self.edge
        )
    }
}
impl core::error::Error for EdgeSelectionError {}

/// Resolves either side of each selected edge to one canonical representative.
///
/// Unlike [`canonicalize_edge_set`], this checks mesh liveness and maps twins.
/// It does not edit the mesh. Use the result when batching kernel attribute writes.
pub fn resolve_edge_set(
    mesh: &exedra_mesh::Mesh,
    edges: &[HalfEdgeId],
) -> Result<ResolvedEdgeSet, EdgeSelectionError> {
    let mut input = edges.to_vec();
    let mut canonicalization_steps = u8::from(canonicalize_edge_set(&mut input));
    let mut edges = Vec::with_capacity(input.len());
    for edge in input {
        edges.push(
            mesh.canonical_edge(edge)
                .ok_or(EdgeSelectionError { edge })?,
        );
    }
    canonicalization_steps += u8::from(canonicalize_edge_set(&mut edges));
    Ok(ResolvedEdgeSet {
        edges,
