...
//!
//! The stage rides the kernel's existing machinery: [`exedra_mesh::op::split_edge`]
//! (attribute propagation via [`exedra_mesh::PropagatePolicy`]) inserts on-edge
...
fn split_one_face(
    ...
    let boundary_attrs: Vec<(VertexId, VertexId, Option<f32>, Option<bool>)> = {
        ...
        let loop_edges: Vec<exedra_mesh::HalfEdgeId> = mesh.face_loop(face).collect();
