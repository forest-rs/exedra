...
## Decision
Cambium relies on Exedra-provided **built-in attribute keys** for core layers (vertex positions, corner UVs, and later corner normal overrides). Cambium does not invent identity for these layers.
...
- **Cambium string keys**: easy to start but leads to drift and ambiguity.
