...
Add `exedra_ops::assembly`, backed by the existing workspace
`exedra_assembly` crate.

Fence: `exedra_ops::assembly` owns deterministic repeat/distribute planning and
...
in `NodeKind::Mirror`. Exedra Ops can then register or reuse a distinct named
recipe-backed part and place it using only proper-rigid transforms.

## Consequences

- `exedra_ops` gains one existing-workspace production dependency on
