...
//! - feature-gated typed workflow adapters for analytic, constructive, and
//!   assembly heads,
//! - explicit cross-domain conversion helpers when an adapter is enabled,
...
//!
//! The intended public surface is this crate root (`exedra_ops::...`) through
//! re-exported operator/runtime types and functions.
//!
//! For manual docs, see:
//! - [`manual::reference`] for the frozen v0.1 operator reference
...
compile_error!("exedra_ops requires either the `std` or `libm` feature");
...
mod context;
#[cfg(any(feature = "analytic", feature = "constructive"))]
pub mod convert;
mod delete;
mod diag;
mod dirty;
