...
This document is a handoff spec for implementing a new crate **`exedra_primitives`**: a small library of deterministic mesh primitive generators that produce **Exedra modeling meshes** plus **semantic selections** and **region tags**. The goal is to provide “hello world” content that immediately exercises Exedra’s invariants, validation, extraction, and (later) booleans — and that plugs naturally into Exedra Edit mesh workflows (like the Byzantine basilica ruin demo).
...
`exedra_primitives` should be usable from `exedra_testkit` and `exedra_edit_testkit` later.
...
- **Do not** require UVs by default. Many primitives should leave:
  - `exedra_mesh::attr::CORNER_UV` absent/missing, so Exedra Edit UV operators can be validated.
...
4. Run Exedra Edit `uv_planar` later against `quad` and `box` and validate UV results (future)
...
- The returned semantic selections are sufficient to drive early Exedra Edit mesh operators (UV projection, sharpness tagging, region selection).
