# exedra_edit_web_viewer

Local-first Three.js viewer for deterministic Exedra Edit scenario snapshots.

This viewer is the interactive web demo surface for Exedra Edit scenarios. It runs
...
```sh
cd apps/exedra_edit_web_viewer
npm install
npm run dev
```

`predev` builds wasm bridge assets from `../exedra_edit_web_bridge` into
`src/wasm_pkg/`.
...
The deployable output is `apps/exedra_edit_web_viewer/dist/` and includes:

- `index.html`
- hashed JS/CSS assets
- wasm bundle from `exedra_edit_web_bridge`
...
- `exedra_primitives requires either std or libm` during wasm build:
  ...
  `exedra_edit`, and `exedra_primitives`; the bridge uses that deterministic
